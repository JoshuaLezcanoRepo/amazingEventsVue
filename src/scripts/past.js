...
const app = createApp({
    ...
    methods: {
        ...
        finalData(data) {
            ...
            this.allCategories = [...new Set(this.pastEvents.map(event => event.category))];
            this.allImages = [...new Set(this.pastEvents.map(event => event.image))];
        },
        clickBtn() {
    ...
    computed: {
        ...
        filterResults() {
            this.contadorCards = this.pastEvents.length;
            if (this.inputCategory.length === 0) {
                this.eventsFiltered = this.pastEvents.filter(event => {
